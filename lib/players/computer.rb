module Players
    class Computer < Player
        ...
        def win(board)
            ...
            winning_combo = comlete_combo?(board, self.token)
            if winning_cmbo && winning_combo.count{|index| board.position(index+1) == self.token} == 2
                puts "...found winning combo #{winning_combo}"
                winning+combo.detect{|indes| !board.taken?(index+1)}
            end
        end
